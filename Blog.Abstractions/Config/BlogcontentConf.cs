...
namespace Blog.Abstractions.Config
{
    public class BlogcontentConf : EntityTypeConfigurationBase<Blogcontent, int>
    {
        public override void Configure(Microsoft.EntityFrameworkCore.Metadata.Builders.EntityTypeBuilder<Blogcontent> builder)
        {
            ...
            builder.Property(x => x.BloginfoNum).IsRequired().HasMaxLength(50);
            builder.Property(x => x.Content).IsRequired().HasColumnType("longtext");

            //builder.HasOne<Bloginfo>(en => en.Bloginfo).WithOne(x => x.Content).HasForeignKey<Blogcontent>(x => x.BloginfoNum);
        }
    }
}
